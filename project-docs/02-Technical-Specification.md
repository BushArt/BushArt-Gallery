...
- **Next.js 16**, App Router only (no Pages Router). The current stable line ships Turbopack as the default bundler, React 19.2 support, and an opt-in caching model via Cache Components (see §8). Minimum Node.js runtime: **20.9+**; local development and CI are pinned to **24.14.1** (see `10-Deployment-Guide.md` §1).
